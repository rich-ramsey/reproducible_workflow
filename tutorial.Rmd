---
title: "Towards a more reproducible scientific workflow in R and R Studio: A tutorial"
author: "Rich"
date: "`r Sys.Date()`"
output:
  html_document:
...
A foundation of science is that it should be open and reproducible by others. 
However, that's quite a tricky goal to achieve and not many people do so on a routine basis (at least not in psychology and human neuroscience disciplines where I have most experience).
...
However, concrete examples of how you combine renv(), git, GitHub and papaja into one workflow seem relatively rare.
...
- If a specific package version has been previously downloaded into the cache, your R project will just link to the project from the cache, rather than re-download it.

That all sounds very sensible to me, but I'm not an engineer, so what do I know?

## Version control via git ##

For those like me that use R Studio and would not consider themselves to be a particularly experienced programmer, then read this outstanding tutorial on git with R Studio: [Vuorre & Curley, 2018](https://journals.sagepub.com/doi/abs/10.1177/2515245918754826).

This tutorial walks you through the value of using version control via git, as well as how to get it done when you primarily use R Studio.
...
It takes a little bit of getting used to, but the benefits are immense because you are then using a tool that was designed for version control to do version control.
And that makes sense in my mind in general, and especially compared to the alternatives (e.g., manually renaming files).
As such, you benefit from someone else doing the hard work to build the relevant infrastructure.
...
- At that point, the online version is the same as the location version.
...
## Prepare a manuscript in R Markdown using the papaja() package ##

papaja is a great package that let's you create a manuscript using R Markdown and format it according to the American Psychological Association (APA) guidelines.
You do not need to use papaja to write manuscript in R markdown and you definitely do not need to follow the APA guidelines.
But since my lab does research in experimental psychology and cognitive neuroscience, it seemed a good place to start.
The main benefits stem from using R Markdown to write manuscripts.
papaja is just one very good example package to help you achieve that aim.

Some of the benefits of using R Markdown to write manuscripts are as follows:

- computer code and prose can be written in one document.
- you can output to a range of different file formats, such as .html, word or pdf.
- changes can be tracked via git just like another file.

Further details can be found in the following links. 

papaja package: https://github.com/crsh/papaja/tree/main

papaja manual: https://frederikaust.com/papaja_man/

My advice would be to read the manual from start to finish as it isn't overly long, but it is really helpful and instructive.
Then I would add in a few extras that I mentioned below in my lab's workflow.

The main emphasis from the papaja manual is having one big document to analyse data and write a manuscript, which spans data, code, prose and manuscript formatting. 
For some projects, this seems fine and computationally efficient to have everything integrated. 
But for other projects, including most of the projects that my lab works on, I find that there is just too much for one document and I get overwhelmed.
Instead, I tend to encourage a modular approach (see next section for details).

# Project organisation #

So far, we've covered the main component pieces that we are using to create a reproducible environment in R, track changes via git, collaborate and share code via github and write a manuscript in R Markdown with papaja.

But how should we structure things to put all of that together?
We have some choices here, two of which I refer to as "monsters" and "modules".

## Monsters vs. modules ##

A "monster" structure would be one massive beast of an R Markdown file that takes in the raw data, runs analyses and produces the manuscript via papaja.
...
Monster workflows are self-contained in one .Rmd file, which is appealing for linking data and model output to plots and tables, as they are all created in one single file. 
...
This is mainly because the thought of having all of that code in one .Rmd file makes my brain melt. 
It is also because some of the Bayesian models we build can take hours, days or weeks to run. 
I know you can use the "cache" function in R Markdown chunks to save re-doing costly computations.
But at the moment, I just prefer to split up the main tasks into separate modules and scripts. 
As such, this is the workflow that I outline below to provide a concrete example.
...
It will also help my lab members (or anyone else) to get their hands dirty in the weeds of a typical data analysis pipeline.
So, as you read what follows, you can head to the /exp1/ folder and take a look at the scripts themselves.
...
- A renv.lock file.
- A /renv/ folder
- A readme.md file. readme files are very important. I would make one for **EVERY SINGLE** project. 
...
A tex file that is produced when outputing to pdf. 

**r-references.bib**

A biblatex file for automatically citing the loaded r packages. 
See the papaja tutorial for details.

**references.bib**

A biblatex file for storing all other cited references.

### settings to modify in the papaja template ###

As the papaja manual suggests, I would recommend:
...
- installing all of the addins for R Studio that the manual suggests.

I would also suggest making a few tweaks to the YAML template, which are as follows (these can also be seen by opening the manuscript.Rmd file, of course):

```{r modify-papaja-yaml, eval=FALSE, echo=TRUE}
...
Changing the bibliography setting means that you need to create a fresh "references.bib" file and place in the folder. 
Code in the manuscript file will automatically access the r-references.bib file and cite any loaded R packages for you. 
The reference.bib file will record any additional citations as they are inserted into the document.

setting floatsintext to yes, just means figures and tables will be presented in the text rather than at the end of the document.

setting header-includes to :raggedbottom makes the placing of text and figures as I think you might want it. 
...
Being able to understand and reproduce past work in an efficient and effective manner is equally as important for your future self, as well as collaborators inside and outside of the lab.
...
But I can talk from experience, that it gets easier relatively quickly and it is not long before you can see the obvious benefits.
...
The benefits hugely outweigh the costs.

## Is writing in R markdown actually any better than writing in MS Word? ##

I don't have that much experience writing manuscripts in R Markdown, so it is new to me at this point.
My initial sense into writing manuscripts in this way along with reading a bunch of blog posts is as follows.
...
- Follow principles of behaviour change and do not try to adopt everything at once.
...
The gradual build might be as follows:
  ...
  + then start using papaja to write manuscripts.

## And what about collaboration when writing in R Markdown? ##

R markdown can generate a word doc, amongst a range of other documents. 
So that's good. 
But, if your collaborators only use MS Word and record edits via "track changes", then you might have to re-do edits in R markdown. 
...
I opted to incorporate more edits into R Markdown manually and version control those changes, in order to benefit from the much more substanial gains in computational reproducibility that R Markdown offers.
But I readily accept that that might not be the same calculation for everyone or for every project.
In some cases, it might just be easier to use MS Word or some other package that is more familiar to the vast majority of the research team.
...
If you add the following two lines of code, git will not ignore the renv.lock file and snapshot will record everything.
This is the workflow that works for me because AFAIK, this means that changes to the renv.lock file are recorded in git. 
And that means that going back in version time is also possible for the renv.lock file and the associated packages.
...
you can select renv::restore() to install packages that are contained in the project's lock.file.
